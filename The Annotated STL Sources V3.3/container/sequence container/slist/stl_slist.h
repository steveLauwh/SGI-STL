...
// 将 __new_node 插入到 __prev_node 后面
...
{
  _Slist_node_base* __before_last = __slist_previous(__head, 0); // 找到空节点的前驱，就是链表尾端
...
struct _Slist_iterator_base
{
  ...
  _Slist_node_base* _M_node; // 指向节点的结构

  _Slist_iterator_base(_Slist_node_base* __x) : _M_node(__x) {}
  void _M_incr() { _M_node = _M_node->_M_next; } // 向前
...
struct _Slist_iterator : public _Slist_iterator_base
{
  ...
  reference operator*() const { return ((_Node*) _M_node)->_M_data; } // 重载 *
...
public:
  ...
  slist& operator= (const slist& __x); // 重载赋值操作符=
...
public:
  ...
  iterator begin() { return iterator((_Node*)this->_M_head._M_next); }  // 迭代器指向链表的第一个节点
  const_iterator begin() const 
    { return const_iterator((_Node*)this->_M_head._M_next);}

  iterator end() { return iterator(0); } // 迭代器指向链表最后一个节点的下一个节点，为空
  ...
  size_type size() const { return __slist_size(this->_M_head._M_next); } // 求出链表的长度
  
  size_type max_size() const { return size_type(-1); }  // 支持的最大元素个数

  bool empty() const { return this->_M_head._M_next == 0; } // 判断链表是否为空

  void swap(slist& __x)  // 交换内容
    { __STD::swap(this->_M_head._M_next, __x._M_head._M_next); }

public:

  reference front() { return ((_Node*) this->_M_head._M_next)->_M_data; } // 访问第一个元素
  const_reference front() const 
    { return ((_Node*) this->_M_head._M_next)->_M_data; }
  void push_front(const value_type& __x)   {  // 在容器开头插入一个元素
  ...
  iterator previous(const_iterator __pos) { // 返回指定位置的前驱
...
private:
  _Node* _M_insert_after(_Node_base* __pos, const value_type& __x) { // 在 __pos 位置后插入 __x
  ...
  // 在 __pos 位置后，插入 [__first, __last) 指向内容
  template <class _InIter>
  void _M_insert_after_range(_Node_base* __pos,
                             _InIter __first, _InIter __last,
                             __false_type) {
